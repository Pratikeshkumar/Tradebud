...
import Insta_Story from "../Status/Insta_Story";
const window = {
...
const BottomHome = ({}) => {
  ...
  useEffect(() => {
    if (depositedMoney && widthdrawMoney) {
      const totalDeposits = depositedMoney.reduce(
        (total, deposit) => total + parseInt(deposit),
        0
      );
      const totalWithdrawals = widthdrawMoney.reduce(
        (total, withdrawal) => total + parseInt(withdrawal),
        0
      );
    ...
    <>
      ...
      >
        ...
        <SafeAreaView style={{ backgroundColor: "rgba(0, 0, 0, 0.2)" }}>
          <View style={{ width: window.width * 1 }}>
            {joined_post && (
              <FlatList
                data={joined_post}
